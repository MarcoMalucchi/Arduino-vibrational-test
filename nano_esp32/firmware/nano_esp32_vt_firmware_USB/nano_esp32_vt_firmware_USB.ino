/*
HERE THE COMPLETE FIRMWARE OF THE SYSTEM: STEPPER MOTOR + PHOTOCELLS MACHINE AND ACCELEROMETER MACHINE NANO-ESP32 ADAPETED

The only difference with the R4 version is the package used to activate the pwm mode at the STEP_PIN: LEDC PWM
...
const int DIR_PIN = 5;  // to control the direction of rotation
const int STEP_PIN = 6; // to control the rotation of the motor shaft
const int ENABLE_PIN = 8; // to enable or disable the driver of the motor, so to stop it or start it
const int PHOTOC_PIN = 7; // photocells pin, in Arudino uno r4 minima only pin D2 and D3 can be used in the interrupt mode
...
const unsigned long FREQUENCY_UPDATE_INTERVAL = 10000;  // The time that pass between subsequent frequency changes in us
...
// NOTE: the incrementations is done with constant acceleration, this means that each increment in frequency is computed as delte_time times acceleration. I can measure the delta_time
// each time I change the frequency or set it as constant, avoiding abdrupt changes, so to have each time the same increment in frequency, then smooth accelerations.
...
// These two below will be the starting reference for time and pulse counts
...
void startMotor() { // Here STOPPED --> RAMPING (this function only changes the logical state, does not actually start the motor)
  ...
  if (targetFrequency == 0.0){  // Meaningful only for the first accension
    targetFrequency = START_FREQUENCY; 
  }

  currentFrequency = START_FREQUENCY; // Everytime the motor restarts it will start rotate at 0.25 Hz

  uint32_t actualFrequency = applyFrequency(currentFrequency);

  if (actualFrequency > 0) {  // prepares the driver and the flag for the actual start up
      digitalWrite(ENABLE_PIN, LOW);
      pwmRunning = true;
  }

  if (currentFrequency < targetFrequency) { // to set the motor state
      motorState = RAMPING;
  }
  else {
...
void stopMotor() {  // Here AT_TARGET/RAMPING --> STOPPED
  ...
  currentFrequency = 0.0; // Here we overwrite the state of the motor so that it will starts from freq = 0 at the next GO, considering the actions performed by startMotor it is just
                          // a matter of logical consistency. It is not necessary

  ledcWriteTone(PWM_CHANNEL, 0);  // Actual switch-off of the STEP_PIN
}
// ------

// ---NON-BLOCKING ACCELERATION FUNCTION (where the motor actually changes its frequency and physically starts rotating)---
void updateFrequency() {
  ...
  // What to do if both the conditions above are not satisfied, i.e. motor ON and target frequency to reach (stateMotor = RAMPING).
  ...
  uint32_t actualFrequency = applyFrequency(currentFrequency); // Here we apply the new frequency of rotation at the motor after the incrementation

  if (!pwmRunning && actualFrequency > 0) { // Here the actions that the function perform when the motor is started after a STOP command (or for the first time)
      digitalWrite(ENABLE_PIN, LOW);
      pwmRunning = true;
  }
...
// ---To set the frequency of the motor---
uint32_t applyFrequency(float frequency) {  // This function compute the number of clock counts corresponding to a complete period of the STEP waveform
  ...
  double actualFrequency = ledcWriteTone(PWM_CHANNEL, stepFrequency); // this function directly set the duty cycle of the waveform to 50%

  // Serial.print("Requested STEP: ");  // debugging
  // Serial.print(stepFrequency);
  // Serial.print(" Hz    LEDC returned: ");
...
bool readRegister(byte reg, byte &value) {  // & here has the function to point at an existing global/local variable used/declaired elsewhere, which value may be changed by
  ...
  byte receiveBytes = Wire.requestFrom(MPU_ADDRESS, (uint8_t)1);  // NOTE: with the nano it is necessary to declaire the type of the number of bytes which will be read in the register
...
void dataReader() { // Modified to fill the packet instead of converting the data read from the device in float
  ...
  byte receivedBytes = Wire.requestFrom(MPU_ADDRESS, (uint8_t)6);
...
void setup() {
  ...
  ledcSetup(PWM_CHANNEL, START_FREQUENCY * STEPS_PER_REVOLUTION, PWM_RESOLUTION);

  ledcAttachPin(STEP_PIN, PWM_CHANNEL);

  ledcWrite(PWM_CHANNEL, 0);  // STEP output stopped
...
void loop() { // What to do while the system is ON.
  ...
  frequencyMeter(); // Thirdly measure the frequency from the photocells signal.

  if (accelerometerState == INITIALIZING) { // Then if the MPU is initialized put it in READY state
    if (initializeMPU()) {
      accelerometerState = READY;
    }
  }
  
  else if (accelerometerState == MEASURING) { // Else if it is measuring start reading its data
    dataReader();
  }
