/*
In this sketch we substitute the design of the updatemotor() function so that we use the PWM mode of the Arduino pins saving hardware time. Unluckly it reveas itself too slow
and this cause the motor to do not accelerate smoothly loosing syncronism during acceleration at high frequency. We have to slightly change the design moving to FspTimer module
*/
...
void updateFrequency() {
  ...
  float stepFrequency = STEPS_PER_REVOLUTION * currentFrequency; // conversion frequency --> steps/second, which is the actual frequency of the waveform in output at STEP_PIN

  stepPWM.get_timer()->set_frequency(stepFrequency);

  stepPWM.pulse_perc(duty_cycle);
...
//   // halfperiod = 1000000.0 / (2.0 * stepFrequency);  // halperiod computation no longer needed since we are implementing the PWM-step-generation control
