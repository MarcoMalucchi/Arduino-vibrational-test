/*
HERE THE FIRST VERSION OF THE STEPPER-MOTOR CONTROLLER WITH PHOTOCELLS TO ACTIVELLY MEASURE ITS FREQUENCY
...
The design of the stepper-motor controller as state machine stays the same. We just add a ISR that counts the passages of the mass and a function frequencyMeter which measure every
5 seconds the rotational frequency of the shaft from the counts of passages and a couple of time measurements

For now the function continuoslly measure the frequency also if the motor is STOPPED (obviously the function must not change the state of the machine), but later the local float
variable frequency will be changed as global, so that, when the status of the motor will be print, there will be also a MEASURED FREQUENCY togheter with the rest, so to have a complete
overview of the state of the motor. That will be usefull also in view of the addition of the accelerometer controller.
*/
...
// Variables needed to measure the frequency from the photocells's signal
volatile unsigned long pulseCount = 0;  // the pulse counter variable shared between ISR and loop()

unsigned long previousTime = 0; // Variables needed by frequencyMeter()
unsigned long previousPulseCount = 0;

const unsigned long MEASUREMENT_INTERVAL = 5000; // The amount of time which has to pass between subsequent frequency measurements
